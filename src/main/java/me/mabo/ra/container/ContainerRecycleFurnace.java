...
public class ContainerRecycleFurnace extends Container {

    // Stores the tile entity instance for later use
    private TileRecycleFurnace tileRecycleFurnace;
    ...
    public ContainerRecycleFurnace(InventoryPlayer invPlayer, TileRecycleFurnace tileRecycleFurnace) {
        this.tileRecycleFurnace = tileRecycleFurnace;
        ...
        final int FUEL_SLOTS_XPOS = 77;
        final int FUEL_SLOTS_YPOS = 68;
        // Add the tile fuel slots
        for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
            int slotNumber = x + FIRST_FUEL_SLOT_NUMBER;
            addSlotToContainer(new SlotFuel(tileRecycleFurnace, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
        }
        ...
        for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
            ...
            addSlotToContainer(new SlotSmeltableInput(tileRecycleFurnace, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS+ SLOT_Y_SPACING * y));
        }
        ...
        for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
            ...
            addSlotToContainer(new SlotOutput(tileRecycleFurnace, slotNumber, OUTPUT_SLOTS_XPOS, OUTPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
        }
    }
    ...
    {
        return tileRecycleFurnace.isUsableByPlayer(player);
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int sourceSlotIndex)
    {
        ...
        if (sourceSlotIndex >= VANILLA_FIRST_SLOT_INDEX && sourceSlotIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
            ...
            if (!TileRecycleFurnace.getSmeltingResultForItem(sourceStack).isEmpty()){  //isEmptyItem
                if (!mergeItemStack(sourceStack, FIRST_INPUT_SLOT_INDEX, FIRST_INPUT_SLOT_INDEX + INPUT_SLOTS_COUNT, false)){
                    return ItemStack.EMPTY;  //EMPTY_ITEM;
                }
            }	else if (TileRecycleFurnace.getItemBurnTime(sourceStack) > 0) {
    ...
    public void detectAndSendChanges() {
        ...
        boolean fieldHasChanged [] = new boolean[tileRecycleFurnace.getFieldCount()];
        if (cachedFields == null) {
            cachedFields = new int[tileRecycleFurnace.getFieldCount()];
            allFieldsHaveChanged = true;
        }
        for (int i = 0; i < cachedFields.length; ++i) {
            if (allFieldsHaveChanged || cachedFields[i] != tileRecycleFurnace.getField(i)) {
                cachedFields[i] = tileRecycleFurnace.getField(i);
        ...
        for (IContainerListener listener : this.listeners) {
            for (int fieldID = 0; fieldID < tileRecycleFurnace.getFieldCount(); ++fieldID) {
    ...
    public void updateProgressBar(int id, int data) {
        tileRecycleFurnace.setField(id, data);
    }
    ...
    public class SlotFuel extends Slot {
        ...
        public boolean isItemValid(ItemStack stack) {
            return TileRecycleFurnace.isItemValidForFuelSlot(stack);
        }
    }
    ...
    public class SlotSmeltableInput extends Slot {
        ...
        @Override
        public boolean isItemValid(ItemStack stack) {
            return TileRecycleFurnace.isItemValidForInputSlot(stack);
        }
    }
    ...
    public class SlotOutput extends Slot {
        ...
        @Override
        public boolean isItemValid(ItemStack stack) {
            return TileRecycleFurnace.isItemValidForOutputSlot(stack);
        }
    }
}
